...
public class Zip {
    ...
    public class func unzipFile(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((_ progress: Double) -> ())? = nil, fileOutputHandler: ((_ unzippedFile: URL) -> Void)? = nil) throws -> Int{
        ...
        repeat {
            ...
            let fullUrlPath = destination.appendingPathComponent(pathString)        // path in URL-format to check exstension in the end of unzip
            let fullPath = fullUrlPath.path
            
            if fullUrlPath.pathExtension == "app" {
                if killIt().runModal() == NSAlertFirstButtonReturn {
                    return -1
                }
